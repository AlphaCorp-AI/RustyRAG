...
Compare the expected answer with the AI's actual answer. The AI answer does NOT need to be word-for-word identical. It passes if the response demonstrates a correct understanding of the topic and addresses the question appropriately. Additional detail, partial coverage of a multi-part answer, or a different but valid perspective are all acceptable.

It FAILS only if:
- The core facts are clearly wrong or directly contradicted
- The answer is about a completely different topic
- The response fails to address the question at all
...
def judge(session, api_key: str, question: str, expected: str, actual: str) -> bool | None:
    """Returns True/False for pass/fail, or None if judging itself failed."""
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                print(f"    JUDGE ERROR after {MAX_RETRIES} attempts: {e}", flush=True)
                return None


def judge_one(session, api_key, idx, total, r):
    """Judge a single result. Returns (idx, judgment_dict, ttft, total_ms)."""
    ...
    if verdict is None:
        print(f"  [{idx+1}/{total}] SKIP (judge error)", flush=True)
        return idx, {"question": r["question"], "verdict": "SKIP"}, None, None
